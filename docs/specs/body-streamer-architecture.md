...
**サービス名**: body-streamer  
**役割**: ストリーミング制御ハブ（身体）  
**バージョン**: 1.4.1 (Stable NVENC Configuration)  
**最終更新**: 2026-02-02
...
**v1.4.1 アップデート**:
- ✅ dev/ren2ブランチの実績ある設定をバックポート
- ✅ NVIDIA NVENC エンコーダー（`nvenc`）による安定配信
- ✅ OBS自動構成ウィザードの完全抑制
- ✅ 接続リトライロジックと起動時ディレイによる高い安定性
...
3. **永続化**: `/app/shared/voice/speech_{id}.wav` として保存。
...
- **`voice_share`**: `/app/shared/voice`
  ...
  - **クリーンアップ**: `body-streamer` の起動時に、`speech_*.wav`（`speech_0000.wav` を除く）が自動的に削除され、ストレージの圧迫を防ぎます。
