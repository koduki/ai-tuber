...
### 発話・表情
- **`POST /api/speak`**: 音声合成と再生を行い、再生完了まで待機します。
...
### 共有ボリューム
`/app/shared/voice` が OBS との間で共有されている必要があります。Streamer が生成した WAV ファイルを OBS がこのパス経由で読み込み、再生します。
