...
| 役割 | 説明 |
| :--- | :--- |
| **表情（立ち絵）の変更** | 指定された感情に合わせて、OBS 上のソース（画像またはメディアソース/動画）の表示・非表示を切り替えます |
...
| `angry` | `angry` | |
| `silent` | `silent` | **無言状態（待機中）** |

> **注意**: OBS のシーンコレクション内で、これらの名前のソース（画像、メディアソース、またはグループ）が定義されている必要があります。
> 
> **自動リセット**: 本システムは、AIがセリフをすべて話し終えた直後に、自動的に `silent` ソースを表示するように制御されています。これにより、次のコメントを待っている間は動画のループ再生などによる「待機モーション」を表現できます。

---

### 動画（メディアソース）使用時の考慮事項

静止画ではなく動画（MP4, WebM等）を立ち絵として使用する場合、OBS側で以下の設定が推奨されます：

1. **ループ再生**: 「ループ」にチェックを入れないと、動画終了後にフリーズまたは消失します。
2. **非アクティブ時の動作**: 「非アクティブ時にファイルを閉じる」はオフを推奨します。オンにすると、感情切り替えの瞬間に読み込みによる一瞬の黒画面が発生する可能性があります。
3. **透過設定**: 背景を透過させるには、透過情報付きのWebM形式を使用するか、グリーンバック動画を用意してOBSの「クロマキー」フィルタを適用してください。
4. **アクティブ時の再生位置**: 「ソースがアクティブになったときに再生を再開する」をオンにすると、感情が切り替わるたびに動画が冒頭から再生されます。オフの場合は裏で流れ続けるため、切り替えた瞬間のポーズがランダムになります。
