...
import schedulerPkg from '@google-cloud/scheduler';
const { CloudSchedulerClient } = schedulerPkg;

import runPkg from '@google-cloud/run';
const { ServicesClient, JobsClient } = runPkg;

import computePkg from '@google-cloud/compute';
const { InstancesClient } = computePkg;

import cloudBuildPkg from '@google-cloud/cloudbuild';
const { CloudBuildClient } = cloudBuildPkg;

import billingPkg from '@google-cloud/billing-budgets';
const { BudgetServiceClient } = billingPkg;
...
export async function getWorkflowInfo(): Promise<WorkflowInfo[]> {
    const parent = `projects/${config.projectId}/locations/${config.region}`;
    const url = `https://workflows.googleapis.com/v1/${parent}/workflows`;
