...
def test_load_mind_config_success():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Create persona.md to avoid errors during initialization if any
        (mind_dir / "persona.md").write_text("# Test Persona")
        
        # Create loader with FileSystemStorageClient pointing to tmpdir
        storage = FileSystemStorageClient(base_path=tmpdir)
        loader = PromptLoader("test_char", storage_client=storage)
...
def test_load_mind_config_missing_file():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        storage = FileSystemStorageClient(base_path=tmpdir)
        loader = PromptLoader("nonexistent", storage_client=storage)
